...
def _load_toads(stream, with_rxid=True, with_txid=True):
    ...
    toads = []
    skipped = []
    kind = '.toads' if with_txid else '.toad'
    opened = False
    ...
    try:
            ...
            try:
                ...
                raise FileFormatError(
                    "{}: line {} is not a {} record: {!r}".format(
                        name, i + 1, kind, line.strip()[:60])) from None
        ...
        if opened:
            stream.close()
    if skipped and not toads:
        # Every record too short, e.g. a .toad file given where a .toads
        # is expected: an error, not an empty result.
        raise FileFormatError("{}: no {} records{}".format(
            name, kind, ' (a .toad file?)' if with_txid else ''))
    for line_no in skipped:
        logging.warning("skipped line #%d: line's formatting is invalid",
                        line_no)
    return toads
