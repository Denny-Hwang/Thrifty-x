...
**`carrier_energy` / `corr_energy` 크기 주의.** 내부적으로 검출기는
Airspy INT16_IQ 샘플을 `2048.0` (12-bit ADC full scale; 자세한 내용은
[`docs/verification/normalization_divisor.md`](verification/normalization_divisor.md)
참조)로 정규화하므로, complex64 magnitude는 대체로 `[-1, +1]` 범위에
들어옵니다. libairspy의 IQ 보정 FIR이 잠시 이 범위를 초과할 수 있어
가끔 `|z| > 1` 값이 보이는 것은 클리핑 오류가 아니라 정상 동작입니다.
`carrier_energy` / `corr_energy` 를 소비하는 코드는 "대략 단위 스케일"
정도로 처리해야지 `[0, 1]` 하드 범위를 가정해서는 안 됩니다.

### 9.2.1 송신기 식별 (`identify --map`)

`thriftyx identify`는 `txid` 컬럼이 포함된 `.toads` 파일을 생성합니다.
기본값에서는 carrier-bin 히스토그램 클러스터링으로 자동 분류합니다.
자동 분류기 (`thriftyx/identify.py:detect_transmitter_windows`) 는
일반적인 BatRF dual-bin 패턴은 잘 처리하지만,
**송신기 모집단 불균형이 극단적인 경우 (예: 40:1)** 약한 송신기가
사라질 수 있습니다 (자세히는
[`docs/verification/auto_classify_robustness.md`](verification/auto_classify_robustness.md)).

**논문 / 운용용 캡처에는 `--map` 으로 명시적인 주파수 맵을
넘기는 것을 권장:**

```ini
# freqmap.cfg - TX 한 개당 한 줄, 값은 "start - stop" (FFT bin 단위;
# 단위 접미사가 있으면 Hz 로 해석).
1: 100 - 105   # TX1 carrier는 bin 100..105
2: 125 - 130   # TX2 carrier는 bin 125..130

# 수신기별 bin offset (수신기 LO offset이 다를 경우).
# '@' + rxid 형태의 키.
@0: 0
@1: 0
```
...
맵은 `thriftyx.identify.load_freqmap` 이 파싱합니다. 각 TX range는
수신기별 offset을 적용한 뒤 평가됩니다. `carrier_bin + carrier_offset`
이 어느 TX range에도 속하지 않으면 `txid = -1` ("미식별" sentinel) 이
되어 `filter_duplicates` 단계에서 `.toads` 출력에서 제거됩니다. 미식별
검출마다 경고 로그가 남습니다.

자동 분류기는 임시 점검용으로는 무난하지만, 명시적 맵이 권장
운용 워크플로입니다.
...
기본적으로 **통합 Qt 뷰어** 한 창에 두 줄의 탭(블록 번호 / 플롯
종류)으로 모든 (블록 × 플롯) 조합을 보여줍니다 — 원본 thrifty와
동일한 레이아웃입니다.  `pip install -e ".[gui]"` 로 `gui` extra를
설치해야 동작하며, 설치되어 있지 않거나 `--no-gui` 를 지정하면
(블록, 플롯) 조합마다 별도의 matplotlib 창이 열립니다.
`-p overview`부터 시작하세요.
